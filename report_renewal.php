...
$count_today_query = "SELECT COUNT(*) AS count_today 
                      FROM renewals 
                      WHERE user_id IS NOT NULL AND DATE(renewal_date) = CURDATE()";
...
$count_week_query = "SELECT COUNT(*) AS count_week 
                     FROM renewals 
                     WHERE user_id IS NOT NULL AND WEEK(renewal_date, 1) = WEEK(CURDATE(), 1) 
                     AND YEAR(renewal_date) = YEAR(CURDATE())";
...
$count_month_query = "SELECT COUNT(*) AS count_month 
                      FROM renewals 
                      WHERE user_id IS NOT NULL AND MONTH(renewal_date) = MONTH(CURDATE()) 
                      AND YEAR(renewal_date) = YEAR(CURDATE())";
                ...
                <ul class="list-unstyled components">
                    ...
                    <li class="active">
                        ...
                        <ul class="collapse list-unstyled" id="report">
                            ...
                            <li>
                                <a href="report_signup.php" aria-current="true"><i class="fas fa-user-check"></i> <span>การสมัครสมาชิก</span></a>
                            </li>
                            <li>
                                <a class="dropdown-item active" href="report_renewal.php"><i class="fa fa-refresh"></i> <span>การต่ออายุ</span></a>
                            </li>
                        </ul>
                    </li>
                </ul>
        ...
        <div class="container">
            ...
            <div class="row g-4">
                ...
                <div class="form-container">
                    <h3 class="text-center">รายงานสมาชิกต่ออายุ</h3>
                    <tbody>
                        <?php
                        // Query เพื่อดึงข้อมูลจากตาราง renewals
                        $renewals_query = "
                        SELECT r.renewal_id, r.user_id, r.payrate_renewal, r.renewal_fee, r.renewal_date, 
                               r.record_id, CONCAT(u.name, ' ', u.surname) AS recorder_name
                        FROM renewals r
                        LEFT JOIN users u ON r.record_id = u.user_id";
                        $renewals_result = mysqli_query($conn, $renewals_query);
                        ...
                        <div class="table-responsive">
                            <table class="table  table-striped table-bordered mt-4">
                                <thead>
                                    <tr>
                                        <th>รหัสผู้ใช้</th>
                                        <th>ค่าต่ออายุสมาชิก</th>
                                        <th>วันที่ต่ออายุสมาชิก</th>
                                        <th>ผู้ทำรายการ</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php while ($row = mysqli_fetch_assoc($renewals_result)): ?>

                                        <tr>
                                            <td><?php echo htmlspecialchars($row['user_id']); ?></td>
                                            <td><?php echo htmlspecialchars(number_format($row['renewal_fee'], 0)); ?></td>
                                            <td><?php echo htmlspecialchars($row['renewal_date']); ?></td>
                                            <td>
                        ...
                        <?php
                        // คืนพื้นที่หน่วยความจำที่ใช้เก็บผลลัพธ์
                        mysqli_free_result($renewals_result);
                        ?>
