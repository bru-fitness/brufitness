...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $userlevel = 2; // ระดับผู้ใช้ 2 (เทรนเนอร์)
    ...
    if (isset($_FILES["profilePicture"]) && $_FILES["profilePicture"]["error"] == 0) {
        ...
        $picture = basename($_FILES["profilePicture"]["name"]);
        $target_file = $target_dir . $picture;
        if (!move_uploaded_file($_FILES["profilePicture"]["tmp_name"], $target_file)) {
            $response["error"] = "เกิดข้อผิดพลาดในการอัปโหลดรูปภาพ";
    ...
    $sql_last_id = "SELECT user_id FROM users WHERE user_id LIKE '2%' AND userlevel = 2 ORDER BY user_id DESC LIMIT 1";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // ถ้ายังไม่มี user_id ที่ขึ้นต้นด้วย '2' กำหนดค่าเริ่มต้นเป็น 20001
        $new_id = 20001;
    }
...
$sql = "INSERT INTO users (user_id, name, surname, gender, telephone, userlevel, username, password, recorddate, status, picture, record_id) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = mysqli_prepare($conn, $sql);
mysqli_stmt_bind_param($stmt, 'ssssssssssss', $user_id, $name, $surname, $gender, $telephone, $userlevel, $username, $password, $recorddate, $status, $picture, $record_id);
...
<head>
    ...
    <title>เพิ่มรายชื่อเทรนเนอร์</title>
    <!-- Basic meta tags -->
    <meta charset="utf-8">
    ...
    <div class="inner_container">
        ...
        <div class="container">
            <div class="form-container">
                <h3 class="text-center">เพิ่มรายชื่อเทรนเนอร์</h3>
                <form id="trainerForm" action="insert_trainer.php" method="POST" enctype="multipart/form-data">
                    <div class="form-row">
                        <div class="form-group">
                            <label for="profilePicture" class="form-label">อัปโหลดรูปภาพ</label>
                            <input type="file" class="form-control" id="profilePicture" name="profilePicture"
                                accept="image/*" required onchange="showPreview(event);">
                        </div>
                        <!-- แสดงตัวอย่างรูปภาพใต้ input อัปโหลดรูปภาพ -->
                        <div class="form-group">
                            <img id="profilePicturePreview" src="#" alt="Image Preview"
        ...
        function showPreview(event) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    preview.style.display = 'block'; // แสดงรูปภาพที่เลือก
                };
                reader.readAsDataURL(file);
            } else {
                preview.src = '#';
                preview.style.display = 'none'; // ซ่อนรูปภาพถ้าไม่ได้เลือกไฟล์
            }
        }
        </script>
        ...
        document.querySelectorAll('#generalForm input, #generalForm select').forEach(function(input) {
            input.addEventListener('change', function() {
                isFormChanged = true; // ฟอร์มถูกแก้ไข
            });
        });

        // ดักจับเหตุการณ์เมื่อคลิกลิงก์หรือปุ่มที่นำไปหน้าอื่น
        document.querySelectorAll('a, button').forEach(function(el) {
            el.addEventListener('click', function(event) {
                if (isFormChanged) {
                    event.preventDefault(); // ป้องกันการกระทำเดิม
                    showCustomAlert(); // แสดงการแจ้งเตือน
                }
            });
        });

        function showCustomAlert() {
            if (confirm("คุณจะไม่ทำรายการนี้ต่อใช่หรือไม่?")) {
                // ถ้าผู้ใช้กดยืนยันให้ทำงานต่อ
